...
@Autonomous(name = "RobotConeFollower", group = "Auto")
public class RobotConeFollower extends LinearOpMode {

    FtcDashboard dashboard;
    private CameraConfigCone sleeveDetection;
    ...
    public void runOpMode() throws InterruptedException {
        ...
        sleeveDetection = new CameraConfigCone(telemetry);
    ...
    private void Autonomous()
    {
        ...
        while(opModeIsActive()){
            ...
            if(sleeveDetection.getLocation() == CameraConfigCone.Location.CENTER) {
                leftAngleCorr = 0;
                rightAngleCorr = 0;
            }
            else if(sleeveDetection.getLocation() == CameraConfigCone.Location.RIGHT){
                rightAngleCorr = 0.1;
                leftAngleCorr = -0.1;
            }
            else if(sleeveDetection.getLocation() == CameraConfigCone.Location.LEFT){
                rightAngleCorr = -0.1;
                leftAngleCorr = 0.1;
            }
            else if(sleeveDetection.getLocation() == CameraConfigCone.Location.VERY_RIGHT){
                rightAngleCorr = 0.15;
                leftAngleCorr = -0.15;
            }
            else if(sleeveDetection.getLocation() == CameraConfigCone.Location.VERY_LEFT){
                rightAngleCorr = -0.15;
                leftAngleCorr = 0.15;
            }
            else{
